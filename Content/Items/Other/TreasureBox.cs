...
namespace KeybrandsPlus.Content.Items.Other
{
    public abstract class TreasureBox : ModItem
    {
        ...
        {
            tag["TreasureBoxScore"] = score;
        }
        public override void LoadData(TagCompound tag)
        {
            score = tag.Get<int>("TreasureBoxScore");
        }
        public override void ModifyTooltips(List<TooltipLine> tooltips)
        {
            tooltips.Add(new TooltipLine(Mod, "KPlus: TreasureBox Score",
                $"Score: {score} ({KeyUtils.GetRankLetter(score)}-Rank)"));
        }
    }
    #region Basic
